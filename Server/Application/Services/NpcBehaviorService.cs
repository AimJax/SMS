...
public class NpcBehaviorService : INpcBehaviorService
{
    ...
    public async Task<IEnumerable<NpcActionCandidate>> GenerateCandidatesAsync(NpcProfile npc, NpcBehaviorConfig config)
    {
        ...
        // Generate follow candidates
        var followCandidates = await GetCandidateAccountsAsync(npc, config.MaxCandidateAccounts);
        foreach (var account in followCandidates)
        {
            var relevance = _relevanceService.CalculateAccountRelevance(account, interests);
            candidates.Add(new NpcActionCandidate
            {
                ActionType = NpcActionType.Follow,
                TargetAccountId = account.Id,
                BaseScore = relevance,
                Reason = $"Follow relevant account: {account.Username}"
            });
        }
        ...
        // Generate unfollow candidates (if following too many)
        ...
        if (followingCount > config.MaxFollowingBeforeUnfollow)
        {
            var staleFollows = await _context.Follows
                ...
                .Take(config.MaxUnfollowsPerTick * 3)
    ...
    private double GetPersonalityModifier(NpcPersonality personality, NpcActionType actionType)
    {
        ...
        {
            NpcActionType.Follow => 0.15 * personality.Extraversion + 0.1 * personality.Openness - 0.1 * personality.Neuroticism,
    ...
    private double GetAccountTypeModifier(AccountType accountType, NpcActionType actionType)
    {
        ...
        {
            AccountType.OrdinaryUser => actionType switch
            {
                NpcActionType.Follow => 0.2,
                NpcActionType.LikePost => 0.15,
                NpcActionType.Comment => 0.1,
                NpcActionType.CreatePost => 0.1,
                _ => 0.0
            },
            AccountType.Creator => actionType switch
            {
                NpcActionType.CreatePost => 0.4,
                NpcActionType.Comment => 0.2,
                NpcActionType.Follow => 0.15,
                NpcActionType.LikePost => 0.15,
                _ => 0.0
            },
            AccountType.Influencer => actionType switch
            {
                NpcActionType.CreatePost => 0.45,
                NpcActionType.Comment => 0.25,
                NpcActionType.LikePost => 0.2,
                NpcActionType.Follow => 0.1,
                _ => 0.0
            },
            AccountType.Celebrity => actionType switch
            {
                NpcActionType.CreatePost => 0.35,
                NpcActionType.Follow => -0.15,
                NpcActionType.Unfollow => 0.1,
                _ => 0.0
            },
            AccountType.Official => actionType switch
            {
                NpcActionType.CreatePost => 0.4,
                NpcActionType.Follow => 0.1,
                NpcActionType.LikePost => -0.1,
                _ => 0.0
            },
            AccountType.News => actionType switch
            {
                NpcActionType.CreatePost => 0.5,
                NpcActionType.Follow => 0.15,
                NpcActionType.LikePost => 0.05,
                _ => 0.0
            },
            _ => 0.0
        };
